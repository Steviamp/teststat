...
namespace UCStatistics.Services
{
    public class ExcelExportService
    {
        ...
        public byte[] ExportIndividualTicketsToExcel(IEnumerable<IndividualTicketDto> data, string sheetName = "Individual Tickets")
        {
            ...
            worksheet.Cell(1, 13).Value = "Ticket CU";
            worksheet.Cell(1, 14).Value = "Lost";
            worksheet.Cell(1, 15).Value = "Date";
            worksheet.Cell(1, 16).Value = "Time";
            worksheet.Cell(1, 17).Value = "Ticket Time";
            worksheet.Cell(1, 18).Value = "Forward Time";
            worksheet.Cell(1, 19).Value = "End Service Time";
            worksheet.Cell(1, 20).Value = "Total Time";
            worksheet.Cell(1, 21).Value = "Service Time";
            worksheet.Cell(1, 22).Value = "End Time";
            worksheet.Cell(1, 23).Value = "Waiting Time";
            worksheet.Cell(1, 24).Value = "Last Update";
            ...
            foreach (var item in data)
            {
                worksheet.Cell(row, 1).Value = item.BranchCode;
                worksheet.Cell(row, 2).Value = item.BranchName;
                worksheet.Cell(row, 3).Value = item.TicketNr;
                worksheet.Cell(row, 4).Value = item.ServiceName;
                worksheet.Cell(row, 5).Value = item.GoldenClient;
                worksheet.Cell(row, 6).Value = item.PlasticCard;
                worksheet.Cell(row, 7).Value = item.DigitalCard;
                worksheet.Cell(row, 8).Value = item.PACustomer;
                worksheet.Cell(row, 9).Value = item.PersonalQueue;
                worksheet.Cell(row, 10).Value = item.DigitalTicket;
                worksheet.Cell(row, 11).Value = item.Workstation?.ToString() ?? "";
                worksheet.Cell(row, 12).Value = item.UserName?.ToString() ?? "";
                worksheet.Cell(row, 13).Value = item.TicketCU;
                worksheet.Cell(row, 14).Value = item.Lost ? "TRUE" : "FALSE";
                worksheet.Cell(row, 15).Value = item.DateDdMmYyyy.ToString("dd.MM.yyyy");
                worksheet.Cell(row, 16).Value = item.Time?.ToString("HH:mm:ss") ?? "";
                worksheet.Cell(row, 17).Value = item.TicketTime?.ToString("dd.MM.yyyy HH:mm") ?? "";
                worksheet.Cell(row, 18).Value = item.ForwardTime?.ToString("dd.MM.yyyy HH:mm:ss") ?? "";
                worksheet.Cell(row, 19).Value = item.EndOfServiceTime?.ToString("dd.MM.yyyy HH:mm") ?? "";
                worksheet.Cell(row, 20).Value = item.TotalTimeInBranch?.ToString(@"hh\:mm\:ss") ?? "";
                worksheet.Cell(row, 21).Value = item.ServiceTimeForwardSt?.ToString(@"hh\:mm\:ss") ?? "";
                worksheet.Cell(row, 22).Value = item.EndOfServiceTimeFormatted;
                worksheet.Cell(row, 23).Value = item.WaitingTime?.ToString(@"hh\:mm\:ss") ?? "";
                worksheet.Cell(row, 24).Value = item.LastUpdateTime?.ToString("dd.MM.yyyy HH:mm:ss") ?? "";
                row++;
            }
