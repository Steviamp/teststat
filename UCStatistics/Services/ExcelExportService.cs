...
namespace UCStatistics.Services
{
    public class ExcelExportService
    {
        ...
        public byte[] ExportLiveSummaryToExcel(IEnumerable<LiveBranchDto> data, string sheetName = "Live Branch Summary")
        {
            using var workbook = new XLWorkbook();
            var worksheet = workbook.Worksheets.Add(sheetName);
            ...
            worksheet.Cell(1, 1).Value = "Office Nr";
            worksheet.Cell(1, 2).Value = "Branch Name";
            worksheet.Cell(1, 3).Value = "Installed Cashiers";
            worksheet.Cell(1, 4).Value = "Active Cashiers";
            worksheet.Cell(1, 5).Value = "Waiting Customers";
            worksheet.Cell(1, 6).Value = "Served Customers";
            worksheet.Cell(1, 7).Value = "Check Ins";
            worksheet.Cell(1, 8).Value = "Plastic Cards";
            worksheet.Cell(1, 9).Value = "Digital Cards";
            worksheet.Cell(1, 10).Value = "Golden Clients Waiting";
            worksheet.Cell(1, 11).Value = "Golden Clients Served";
            worksheet.Cell(1, 12).Value = "PA Waiting";
            worksheet.Cell(1, 13).Value = "PA Served";
            worksheet.Cell(1, 14).Value = "Digital Tickets";
            worksheet.Cell(1, 15).Value = "AvgWaitingTime";
            worksheet.Cell(1, 16).Value = "AvgServiceTime";
            ...
            foreach (var item in data)
            {
                worksheet.Cell(row, 1).Value = item.OfficeNr;
                worksheet.Cell(row, 2).Value = item.OfficeName;
                worksheet.Cell(row, 3).Value = item.InstalledCashiers;
                worksheet.Cell(row, 4).Value = item.ActiveCashiers;
                worksheet.Cell(row, 5).Value = item.WaitingCustomers;
                worksheet.Cell(row, 6).Value = item.ServedCustomers;
                worksheet.Cell(row, 7).Value = item.NumberOfCheckIns;
                worksheet.Cell(row, 8).Value = item.NumberOfGoldenClientsWaiting;
                worksheet.Cell(row, 9).Value = item.NumberOfGoldenClientsServed;
                worksheet.Cell(row, 10).Value = item.PersonalQueueWaiting;
                worksheet.Cell(row, 11).Value = item.PersonalQueueServed;
                worksheet.Cell(row, 12).Value = item.NumberOfDigitalTickets;
                worksheet.Cell(row, 13).Value = item.AvgWaitingTime;
                worksheet.Cell(row, 14).Value = item.AvgServiceTime;
                row++;
            }
        ...
        public byte[] ExportLiveServiceSummaryToExcel(IEnumerable<LiveServiceDto> data, string sheetName = "Live Service Summary")
        {
            using var workbook = new XLWorkbook();
            var worksheet = workbook.Worksheets.Add(sheetName);
            ...
            worksheet.Cell(1, 2).Value = "Branch Name";
            worksheet.Cell(1, 3).Value = "Active Cashiers";
            worksheet.Cell(1, 4).Value = "Waiting Customers";
            worksheet.Cell(1, 5).Value = "Served Customers";
            worksheet.Cell(1, 6).Value = "Avg Waiting Time";
            worksheet.Cell(1, 7).Value = "Avg Serving Time";
            ...
            foreach (var item in data)
            {
                ...
                worksheet.Cell(row, 2).Value = item.OfficeName;
                worksheet.Cell(row, 3).Value = item.ActiveCashiers;
                worksheet.Cell(row, 4).Value = item.WaitingCustomers;
                worksheet.Cell(row, 5).Value = item.ServedCustomers;
                worksheet.Cell(row, 6).Value = item.AvgWaitingTime;
                worksheet.Cell(row, 7).Value = item.AvgServingTime;
                row++;
            }
