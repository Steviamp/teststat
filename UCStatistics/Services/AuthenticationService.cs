...
namespace UCStatistics.Services
{
    ...
    public class AuthenticationService
    {
        ...
        public ActiveDirectoryUserDto? GetAuthenticatedUser(HttpContext httpContext)
        {
            try
            {
                ...
                if (userRole == UserRole.Supervisor && !string.IsNullOrEmpty(department))
                {
                    ...
                    //if (supervisorBranch == null)
                    //{
                    //    _logger.LogWarning("Supervisor department '{Department}' is not in expected format. Denying access.", department);
                    //    return null;
                    //}
                }
                ...
                _logger.LogInformation("User {UserName} authenticated successfully with role {Role}",
                    userName, userRole);
                _logger.LogWarning("Real Department: {Department}", department);
                _logger.LogWarning("UserName: {UserName}, Role: {Role}, SupervisorBranch: {SupervisorBranch}", userName, userRole, supervisorBranch);
        ...
        private ActiveDirectoryUserDto GetTestUser()
        {
            ...
            var testRole = UserRole.Administrator; // Change to UserRole.Supervisor, UserRole.StatisticsUser, etc.
        ...
        private UserRole GetUserRole(List<string> groupNames)
        {
            ...
            if (groupNames.Any(g => g.Equals("NEMOQ_QMS_Statistics user OPU (AG)", StringComparison.OrdinalIgnoreCase)))
            {
            ...
            //UAT ROLES
            if (groupNames.Any(g => g.Equals("NEMOQ_QMS_Supervisor_Test (AG)", StringComparison.OrdinalIgnoreCase)))
            {
                return UserRole.Supervisor;
            }

            if (groupNames.Any(g => g.Equals("NEMOQ_QMS_Statistics user OPU_Test (AG)", StringComparison.OrdinalIgnoreCase)))
            {
                return UserRole.StatisticsUser;
            }

            if (groupNames.Any(g => g.Equals("NEMOQ_QMS_Administrator OPU_Test (AG)", StringComparison.OrdinalIgnoreCase)))
            {
                return UserRole.Administrator;
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(department) || !department.Contains(" - "))
                return null;
