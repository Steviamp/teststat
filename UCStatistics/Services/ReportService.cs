...
namespace UCStatistics.Services
{
    public class ReportService
    {
        ...
        public Task<IEnumerable<IndividualTicketDto>> GetLiveIndividualTicketDetailsAsync(FilterCriteria criteria, ActiveDirectoryUserDto? currentUser = null)
        {
            var filteredCriteria = ApplyRoleBasedFiltering(criteria, currentUser);
            return _repository.GetLiveIndividualTicketDetailsAsync(filteredCriteria);
        }
        ...
        public async Task<byte[]> ExportLiveIndividualTicketsToExcelAsync(FilterCriteria criteria, ActiveDirectoryUserDto? currentUser = null)
        {
            var filteredCriteria = ApplyRoleBasedFiltering(criteria, currentUser);
            var data = await GetLiveIndividualTicketDetailsAsync(filteredCriteria);
            var excelService = new ExcelExportService();
            return excelService.ExportIndividualTicketsToExcel(data, "Live Individual Tickets");
        }
